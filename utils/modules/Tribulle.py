...
class Tribulle:
    ...
    def sendFriendsList(self, readPacket):
        ...
        if self.client.marriage == "":
            ...
            p.writeInt(rs[1]).writeUTF(rs[0].lower() + self.server.getPlayerTag(rs[0])).writeByte(rs[2]).writeInt(rs[1]).writeByte(1).writeBoolean(self.server.checkConnectedAccount(rs[0])).writeInt(4).writeUTF(player.roomName if player else "").writeInt(rs[3])
        ...
        for playerName in playersNames:
            ...
            p.writeInt(info[0]).writeUTF(playerName.lower() + self.server.getPlayerTag(playerName)).writeByte(genderID).writeInt(info[0]).writeByte(1 if isFriend else 0).writeBoolean(self.server.checkConnectedAccount(playerName)).writeInt(4 if isFriend and player != None else 1).writeUTF(player.roomName1 if isFriend and player != None else "").writeInt(info[4] if isFriend else 0)
        if readPacket == None:
            p.writeShort(len(self.client.ignoredsList))

            for playerName in self.client.ignoredsList:
                p.writeUTF(playerName.lower() + self.server.getPlayerTag(playerName))
    ...
    def addFriend(self, readPacket):
        tribulleID, playerName = readPacket.readInt(), Utils.parsePlayerName(readPacket.readUTF().split("#")[0])
        ...
        self.sendPacket(36, ByteArray().writeInt(rs[1]).writeUTF(Utils.parsePlayerName(playerName) + self.server.getPlayerTag(playerName)).writeByte(rs[2]).writeInt(rs[1]).writeShort(self.server.checkConnectedAccount(playerName)).writeInt(4 if isFriend else 0).writeUTF(player.roomName1 if isFriend and player != None else "").writeInt(rs[3] if isFriend else 0).toByteArray())
        self.sendPacket(19, ByteArray().writeInt(tribulleID).writeByte(1).toByteArray())
        if player != None:
            player.tribulle.sendPacket(35, ByteArray().writeInt(self.client.playerID).writeUTF(self.client.playerName.lower() + self.client.playerTag).writeByte(self.client.gender).writeInt(self.client.playerID).writeByte(1).writeByte(self.server.checkConnectedAccount(self.client.playerName)).writeInt(4 if isFriend else 0).writeUTF(self.client.roomName1 if isFriend else "").writeInt(self.client.lastOn if isFriend else 0).toByteArray())
    ...
    def removeFriend(self, readPacket):
        ...
        if playerName in self.client.friendsList:
            ...
            if player != None:
                player.tribulle.sendPacket(35, ByteArray().writeInt(self.client.playerID).writeUTF(self.client.playerName.lower() + self.client.playerTag).writeByte(self.client.gender).writeInt(self.client.playerID).writeShort(1).writeInt(0).writeUTF("").writeInt(0).toByteArray())

    def sendFriendConnected(self, playerName):
        if playerName in self.client.friendsList:
            player = self.server.players.get(playerName)
            self.sendPacket(35, ByteArray().writeInt(player.playerID).writeUTF(playerName.lower() + self.server.getPlayerTag(playerName)).writeByte(player.gender).writeInt(player.playerID).writeByte(1).writeByte(1).writeInt(1).writeUTF("").writeInt(player.lastOn).toByteArray())
            self.sendPacket(32, ByteArray().writeUTF(player.playerName.lower() + player.playerTag).toByteArray())
    ...
    def sendFriendChangedRoom(self, playerName, langueID):
        if playerName in self.client.friendsList:
            ...
            if player == None:
                 return
            self.sendPacket(35, ByteArray().writeInt(player.playerID).writeUTF(playerName.lower() + self.server.getPlayerTag(playerName)).writeByte(player.gender).writeInt(player.playerID).writeByte(1).writeByte(1).writeInt(4).writeUTF(player.roomName1).writeInt(player.lastOn).toByteArray())
    ...
    def sendFriendDisconnected(self, playerName):
        if playerName in self.client.friendsList:
            ...
            self.sendPacket(35, ByteArray().writeInt(rs[1]).writeUTF(playerName.lower() + self.server.getPlayerTag(playerName)).writeByte(rs[2]).writeInt(rs[1]).writeByte(1).writeByte(0).writeInt(1).writeUTF("").writeInt(rs[3]).toByteArray())
            self.sendPacket(33, ByteArray().writeUTF(playerName.lower() + self.server.getPlayerTag(playerName)).toByteArray())
    ...
    def sendIgnoredsList(self, readPacket):
        ...
        for playerName in self.client.ignoredsList:
            packet.writeUTF(playerName + self.server.getPlayerTag(playerName))
    ...
    def whisperMessage(self, readPacket):
        tribulleID, playerName, message = readPacket.readInt(), Utils.parsePlayerName(readPacket.readUTF().split("#")[0]), readPacket.readUTF().replace("\n", "").replace("&amp;#", "&#").replace("<", "&lt;")
            ...
            if can:
                ...
                if player != None:
                    ...
                    if not (self.client.playerName in player.ignoredsList) and not isCheck:
                        player.tribulle.sendPacket(66, ByteArray().writeUTF(self.client.playerName.lower() + self.client.playerTag).writeInt(self.client.langueID+1).writeUTF(player.playerName.lower() + player.playerTag).writeUTF(message).toByteArray())
                    self.sendPacket(66, ByteArray().writeUTF(self.client.playerName.lower() + self.client.playerTag).writeInt(player.langueID+1).writeUTF(player.playerName.lower() + player.playerTag).writeUTF(message).toByteArray())
    ...
    def marriageInvite(self, readPacket):
        ...
        if not self.server.checkConnectedAccount(playerName) or not self.server.checkExistingUser(playerName):
            ...
            if not self.client.playerName in player.ignoredMarriageInvites:
                ...
                player.tribulle.sendPacket(38, ByteArray().writeUTF(self.client.playerName + self.client.playerTag).toByteArray())
                self.sendPacket(23, packet.writeByte(1).toByteArray())

    def marriageAnswer(self, readPacket):
        tribulleID, playerName, answer = readPacket.readInt(), Utils.parsePlayerName(readPacket.readUTF().split("#")[0]), readPacket.readByte()

        player = self.server.players.get(playerName)
        if player != None:
            if answer == 0:
                self.sendPacket(25, ByteArray().writeInt(tribulleID).writeByte(1).toByteArray())
                player.tribulle.sendPacket(40, ByteArray().writeUTF(self.client.playerName.lower() + self.client.playerTag).toByteArray())
                ...
                self.sendPacket(39, ByteArray().writeUTF(player.playerName.lower() + player.playerTag).toByteArray())
                player.tribulle.sendPacket(39, ByteArray().writeUTF(self.client.playerName.lower() + self.client.playerTag).toByteArray())
    ...
    def marriageDivorce(self, readPacket):
        ...
        self.sendPacket(41, ByteArray().writeUTF(self.client.marriage + self.server.getPlayerTag(self.client.marriage)).writeByte(1).toByteArray())
        player = self.server.players.get(self.client.marriage)
        if player != None:
            player.tribulle.sendPacket(41, ByteArray().writeUTF(player.marriage + self.server.getPlayerTag(player.marriage)).writeByte(1).toByteArray())
            player.marriage = ""
    ...
    def sendTribe(self, isNew):
        ...
        self.sendPacketWholeTribe(29, ByteArray().writeInt(self.client.tribeChat).writeInt(self.client.playerID).writeUTF(self.client.playerName.lower() + self.client.playerTag).toByteArray())
        self.sendTribeInfo()
    ...
    def sendLoginMessageTribe(self):
        ...
        self.Cursor.execute("select PlayerID, Gender, LastOn, TribeRank, TribeJoined from Users where Username in (%s)" %(Utils.joinWithQuotes(members)))
        for rs in self.Cursor.fetchall():
            infos[member] = [rs[0], rs[1], rs[2], rs[3], rs[4]]
        ...
        for member in members:
            ...
            packet.writeUTF(member.lower() + self.server.getPlayerTag(member))
            ...
            packet.writeInt(4)
            packet.writeUTF(player.roomName1 if player != None else "")
    ...
    def sendTribeMemberConnected(self):
        self.sendPacketWholeTribe(88, ByteArray().writeUTF(self.client.playerName.lower() + self.client.playerTag).toByteArray(), True)
        self.sendPacketWholeTribe(131, ByteArray().writeInt(self.client.playerID).writeUTF(self.client.playerName.lower() + self.client.playerTag).writeByte(self.client.gender).writeInt(self.client.playerID).writeInt(0).writeByte(self.client.tribeRank).writeInt(1).writeUTF("").toByteArray())

    def sendTribeMemberChangeRoom(self):
        self.sendPacketWholeTribe(131, ByteArray().writeInt(self.client.playerID).writeUTF(self.client.playerName.lower() + self.client.playerTag).writeByte(self.client.gender).writeInt(self.client.playerID).writeInt(0).writeByte(self.client.tribeRank).writeInt(4).writeUTF(self.client.roomName1).toByteArray())

    def sendTribeMemberDisconnected(self):
        self.sendPacketWholeTribe(90, ByteArray().writeUTF(self.client.playerName.lower() + self.client.playerTag).toByteArray())
        self.sendPacketWholeTribe(131, ByteArray().writeInt(self.client.playerID).writeUTF(self.client.playerName.lower() + self.client.playerTag).writeByte(self.client.gender).writeInt(self.client.playerID).writeInt(self.client.lastOn).writeByte(self.client.tribeRank).writeInt(1).writeUTF("").toByteArray())

    def sendPlayerInfo(self):
        self.sendPacket(166, ByteArray().writeInt(0).writeInt(self.client.playerID).writeInt(self.client.playerID).writeInt(self.getInGenderMarriage(self.client.playerName)).writeInt(self.server.getPlayerID(self.client.marriage) if not self.client.marriage == "" else 0).writeUTF(self.client.marriage + self.server.getPlayerTag(self.client.marriage)).toByteArray())
    ...
    def tribeInvite(self, readPacket):
        ...
        if len(self.getTribeMembers(self.client.tribeCode)) >= 1000:
            ...
            player.tribulle.sendPacket(86, ByteArray().writeUTF(self.client.playerName.lower() + self.client.playerTag).writeUTF(self.client.tribeName).toByteArray())
            self.sendPacket(79, ByteArray().writeInt(tribulleID).writeByte(1).toByteArray())

    def tribeInviteAnswer(self, readPacket):
        tribulleID, playerName, answer = readPacket.readInt(), readPacket.readUTF().split("#")[0], readPacket.readByte()
        ...
        if self.client.tribeCode != 0:
            ...
            if answer == 0:
                ...
                player.tribulle.sendPacket(87, ByteArray().writeUTF(self.client.playerName.lower() + self.client.playerTag).writeByte(0).toByteArray())
                ...
                player.tribulle.sendPacket(87, ByteArray().writeUTF(self.client.playerName + self.client.playerTag).writeByte(1).toByteArray())
                self.sendPacketWholeTribe(91, ByteArray().writeUTF(self.client.playerName + self.client.playerTag).toByteArray(), True)
                for member in members:
                    player = self.server.players.get(member)
                    if player != None:
    ...
    def changeTribeMessage(self, readPacket):
        ...
        self.sendTribeInfo()
        self.sendPacketWholeTribe(125, ByteArray().writeUTF(self.client.playerName.lower() + self.client.playerTag).writeUTF(message).toByteArray(), True)
    ...
    def changeTribePlayerRank(self, packet):
        tribulleID, playerName, rankID = packet.readInt(), packet.readUTF().split("#")[0], packet.readByte()
        ...
        self.sendPacket(131, ByteArray().writeInt(rs[1]).writeUTF(playerName.lower() + self.server.getPlayerTag(playerName)).writeByte(rs[2]).writeInt(rs[1]).writeInt(0 if self.server.checkConnectedAccount(playerName) else rs[3]).writeByte(rankID).writeInt(1).writeUTF("" if player == None else player.roomName1).toByteArray())
        self.sendPacketWholeTribe(124, ByteArray().writeUTF(self.client.playerName.lower() + self.client.playerTag).writeUTF(playerName.lower() + self.server.getPlayerTag(playerName)).writeUTF(rankName).toByteArray(), True)
    ...
    def leaveTribe(self, packet):
        ...
        if self.client.tribeRank == (len(self.client.tribeRanks.split(";"))-1):
            ...
            self.sendPacketWholeTribe(92, ByteArray().writeUTF(self.client.playerName.lower() + self.client.playerTag).toByteArray(), True)
    ...
    def kickPlayerTribe(self, packet):
        tribulleID, playerName = packet.readInt(), packet.readUTF().split("#")[0]
        p = ByteArray().writeInt(tribulleID)
        player = self.server.players.get(playerName)
        ...
        if tribeCode != 0:
            ...
            if playerName in members:
                ...
                self.sendPacketWholeTribe(93, ByteArray().writeUTF(playerName.lower() + self.server.getPlayerTag(playerName)).writeUTF(self.client.playerName.lower() + self.client.playerTag).toByteArray(), True)
    ...
    def setTribeMaster(self, packet):
        ...
        self.sendPacket(131, ByteArray().writeInt(rs[1]).writeUTF(playerName.lower() + self.server.getPlayerTag(playerName)).writeByte(rs[2]).writeInt(rs[1]).writeInt(0 if self.server.checkConnectedAccount(playerName) else rs[3]).writeByte(len(rankInfo)-1).writeInt(4).writeUTF("" if player == None else player.roomName1).toByteArray())
        self.sendPacket(131, ByteArray().writeInt(self.client.playerID).writeUTF(self.client.playerName.lower() + self.client.playerTag).writeByte(self.client.gender).writeInt(self.client.playerID).writeInt(0).writeByte(len(rankInfo)-2).writeInt(4).writeUTF(self.client.roomName1).toByteArray())
    ...
    def finishTribe(self, packet):
        ...
        for member in members:
            ...
            if player:
                ...
                player.tribeRanks = ""
                player.tribeInvite = []
                player.tribulle.sendPacket(93, ByteArray().writeUTF(player.playerName.lower() + player.playerTag).writeUTF(self.client.playerName.lower() + self.client.playerTag).toByteArray())
    ...
    def chatMessage(self, packet):
        ...
        self.sendPacketWholeChat(chatName, 64, ByteArray().writeUTF(self.client.playerName.lower() + self.client.playerTag).writeInt(self.client.langueID+1).writeUTF(chatName).writeUTF(message).toByteArray(), True)
    ...
    def chatMembersList(self, packet):
        ...
        for player in self.server.players.copy().values():
            if chatName in self.server.chats and self.client.playerName in self.server.chats[chatName]:
                p.writeUTF(player.playerName + player.playerTag)
        self.sendPacket(59, p.toByteArray())

    def sendTribeChatMessage(self, readPacket):
        tribulleID, message = readPacket.readInt(), readPacket.readUTF()
        self.sendPacketWholeTribe(65, ByteArray().writeUTF(self.client.playerName.lower() + self.client.playerTag).writeUTF(message).toByteArray(), True)
    ...
    def removeMarriage(self, playerName):
        player = self.server.players.get(playerName)
        if player != None:
            self.server.players[playerName].marriage = 0
        else:
            self.Cursor.execute("update Users set Marriage = '' where Username = %s", [playerName])
    ...
    def setTribeMembers(self, tribeCode, members):
        self.Cursor.execute("update Tribe set Members = %s where Code = %s", [",".join(map(str, [self.server.getPlayerID(member) for member in members])), tribeCode])
